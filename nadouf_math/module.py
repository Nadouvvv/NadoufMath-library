...
class Nadoufmath:
    ...
    def sum_of(self, *args):
        self.number = self.number + sum(args)
        return self
    ...
    def power(self, exponent):
        self.number = self.number**exponent
        return self
    ...
    def lcm_with(self, *args):
        ...
        for num in args:
            num = abs(num)
            from .module import gcd_with as gcd_func

            result = abs(result * num) // gcd_func(result, num)
        self.number = result
        return self
    ...
    def floor(self):
        if self.number >= 0:
            ...
            self.number = -((-self.number) // 1) - (1 if self.number % 1 != 0 else 0)
        return self
    ...
    def tan(self, terms=10):
        ...
        sin_val = self.sin(terms).number
        self.number = original
        cos_val = self.cos(terms).number
        self.number = sin_val / cos_val
        return self
